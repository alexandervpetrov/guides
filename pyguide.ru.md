...
в разработке элементы ФП полезно и разумно. Быть может когда-то всё приложения будут писать
...
Использовать классы при написании кода теоретически можно по крайней мере двумя способами:
1) просто для упаковки данных в объекты, в качестве namespace-ов как альтернатива
...
Лично моя позиция - ООП на проекте не должно быть, кроме случаев когда фреймворк
...
значение (post-operation); 3) делают что-либо более сложное - или и до и после,
или меняют каким-то образом операцию (other-operation); 4) просто маркируют декорируемый объект.
